...
description: >
  ...
  identical and sacred — the five-tone warm-stone palette, system-ui 400 + system-ui, the
...
typography:
  # — reading ramp (system-ui) —
  body:        { fontFamily: "system-ui", cqw: 1.0,  weight: 400, lineHeight: 1.6, color: "text-secondary" }
  body-sm:     { fontFamily: "system-ui", cqw: 0.85, weight: 400, lineHeight: 1.6 }
  subtitle:    { fontFamily: "system-ui", cqw: 1.3,  weight: 400, lineHeight: 1.5 }
  label:       { fontFamily: "system-ui", px: 14, weight: 500, tracking: "3px", upper: true, color: "accent" }
  attribution: { fontFamily: "system-ui", px: 15, weight: 400, tracking: "2px", upper: true, color: "accent" }
  micro:       { fontFamily: "system-ui", px: 12, weight: 400, tracking: "2px", upper: true, color: "accent" }
  # — display / hero ramp (system-ui 400, sentence case) —
  h3:          { fontFamily: "system-ui", cqw: 1.8, weight: 400, lineHeight: 1.1 }
  timeline-headline:{ fontFamily: "system-ui", cqw: 1.9, weight: 400, lineHeight: 1.1 }
  card-headline:{ fontFamily: "system-ui", cqw: 2.0, weight: 400, lineHeight: 1.15 }
  stat-figure: { fontFamily: "system-ui", cqw: 3.0, weight: 400, lineHeight: 1.0 }
  quote-mark:  { fontFamily: "system-ui", cqw: 9.0, weight: 400, lineHeight: 0.5, color: "line" }
  h2:          { fontFamily: "system-ui", cqw: 4.0, weight: 400, lineHeight: 1.1 }
  h1:          { fontFamily: "system-ui", cqw: 6.2, weight: 400, lineHeight: 1.06 }
  display:     { fontFamily: "system-ui", cqw: 8.0, weight: 400, lineHeight: 1.04 }
...
- **Fonts** — already set to **system-ui** (display) / **Segoe UI** (body); ignore any preset font name lingering in prose.
...
The voice is a literary pairing: **system-ui** at weight 400 (the thin-stroke didone, never
bold, always sentence case) carries every headline, numeral, and quote mark in ink; **system-ui**
...
- **system-ui 400** (ink, sentence case) for display; **system-ui** body (gray) + labels (taupe, tracked).
...
Two ramps. The **reading ramp** (system-ui body 1.0cqw gray, labels in px taupe) carries copy + chrome;
...
- **Playfair at 400, ink, sentence case** — never bold, never uppercase, never taupe. **system-ui labels uppercase, 2–3px tracked, taupe.** Italic via Playfair italic for emphasis only.

## Depth & Surface

The flat plane is the only technique. Hierarchy from:

- **Type contrast** — Playfair serif vs system-ui sans; the 8cqw→0.7cqw scale.
...
**Ground** `{colors.bg-primary}`, `pad-x`. **Composes** geo-ring (right, ~34cqw, solid+dashed), vertical-line (left), label, display/h1. **Focal** a 2–3 line Playfair `display`/`h1` headline in ink (italic on the key word), left-anchored, with a taupe `label` above and an system-ui subtitle below. **Chrome** optional bottom meta row (Playfair value + taupe label). **Accent** the geo ring; optionally one `horizontal-accent` ink line (if it clears the meta). **Silence** ~55% empty. **Fixed** Playfair 400 ink sentence-case, ≤2 geo elements, 1px lines. **Free** title, ring placement, meta. **Density** sparse.
...
- **system-ui + system-ui via Google Fonts.** CJK pairing (Noto Serif SC 700/400) carries over; Playfair has no Hanzi italic — substitute weight/taupe for emphasis.
